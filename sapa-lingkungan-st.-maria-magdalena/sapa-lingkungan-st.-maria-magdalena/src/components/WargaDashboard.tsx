                    ...
                    <div className="p-4 sm:p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-gradient-to-r from-orange-50/80 via-white to-amber-50/50">
                      ...
                      <div className="flex items-center gap-2 self-end md:self-center shrink-0 flex-wrap">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shadow-2xs"
                          title="Cetak atau Simpan PDF KK"
                        >
                                ...
                                {ak.agama === 'Katolik' && (
                                  <div className="bg-amber-50/50 p-2.5 rounded-lg border border-amber-200/60 text-[11px] text-amber-950 space-y-1">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2">
                                      <div>
                                        <span className="font-semibold">Baptis: </span>
                                        {ak.baptis?.nama_baptis ? `${ak.baptis.nama_baptis} (${ak.baptis.tgl_baptis || 'Tgl -'})` : 'Belum Ada'}
                                      </div>
                                      <div>
                                        <span className="font-semibold">Komuni I: </span>
                                        {ak.komuni_pertama?.tgl_komuni_pertama ? ak.komuni_pertama.tgl_komuni_pertama : 'Belum Ada'}
                                      </div>
                                      <div>
                                        <span className="font-semibold">Krisma: </span>
                                        {ak.krisma?.nama_krisma ? `${ak.krisma.nama_krisma}` : 'Belum Ada'}
                                      </div>
                                      <div>
                                        <span className="font-semibold">Perkawinan: </span>
                                        {ak.perkawinan?.tgl_perkawinan ? ak.perkawinan.tgl_perkawinan : 'Belum / Non'}
                                      </div>
                                    </div>
                                  </div>
                                )}
