...
export async function syncFirestoreWithSqlite(sqliteDb: Database): Promise<boolean> {
...
async function uploadSqliteToFirestore(sqliteDb: Database) {
  const tables = [
    'users',
    'kepala_keluarga',
    'anggota_keluarga',
...
async function loadFirestoreIntoSqlite(sqliteDb: Database) {
  const collections = [
  ...
  for (const item of collections) {
    try {
      ...
      // Always clear local SQLite table first to match Cloud Firestore state
      ...
      if (!querySnapshot.empty) {
        querySnapshot.forEach((docSnap) => {
          ...
          try {
            ...
            console.warn(`Error inserting doc ${docSnap.id} into ${item.name}:`, insertErr);
          }
        });
      }
    } catch (e) {
