...
export async function getDb(): Promise<Database> {
  ...
  syncFirestoreWithSqlite(dbInstance).catch((err) => {
...
function seedInitialData(db: Database) {
  ...
  db.run(`
    INSERT INTO anggota_keluarga VALUES (
      ...
      '${JSON.stringify({ no_surat_komper: 'KOM/1988/05/110', tgl_komuni_pertama: '1988-06-01', tempat_tempat: 'Paroki St. Vincentius a Paulo Kediri' })}',
  ...
  // 7. Seed Jadwal Kegiatan
  db.run(`
    INSERT INTO jadwal_kegiatan VALUES (
      'jg-1',
      'Doa Rosario Lingkungan St. Maria Magdalena',
      'Doa Lingkungan',
      '2026-08-05',
      '18:30 WIB',
      'Rumah Bpk. Yohanes Sugeng (RT 02 / RW 05)',
      'Mohon kehadiran seluruh umat keluarga dengan membawa Buku Puji Syukur.',
      '',
      '${now}'
    )
  `);
  db.run(`
    INSERT INTO jadwal_kegiatan VALUES (
      'jg-2',
      'Misa Kudus Pelindung St. Maria Magdalena',
      'Misa Lingkungan',
      '2026-08-20',
      '17:00 WIB',
      'Gereja St. Vincentius a Paulo Kediri',
      'Misa konselebrasi wilayah dan pemotongan tumpeng syukuran.',
      '',
      '${now}'
    )
  `);
