  ...
  kkList.forEach((kk) => {
    ...
    if (members.length === 0) {
      ...
      members.forEach((ak, idx) => {
        const isFirst = idx === 0;
        const baptis = ak.baptis?.nama_baptis ? `${ak.baptis.nama_baptis} (${ak.baptis.tgl_baptis || ''})` : '-';
        const komuni = ak.komuni_pertama?.tgl_komuni_pertama || '-';
        const krisma = ak.krisma?.nama_krisma ? `${ak.krisma.nama_krisma} (${ak.krisma.tgl_krisma || ''})` : '-';
        const nikah = ak.perkawinan?.tgl_perkawinan || '-';
