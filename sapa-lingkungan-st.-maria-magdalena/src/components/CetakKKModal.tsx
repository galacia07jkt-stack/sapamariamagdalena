...
import { KepalaKeluarga, AnggotaKeluarga } from '../types';
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center bg-black/20 p-1 rounded-xl border border-white/20 text-[11px] font-bold">
                ...
                <button
                  type="button"
                  onClick={() => setLogoVariant('svg')}
                  className={`px-2 py-1 rounded-lg transition-all ${logoVariant === 'svg' ? 'bg-amber-400 text-amber-950 font-black shadow-xs' : 'text-white hover:bg-white/10'}`}
                  title="Gunakan Logo Stempel Vector SVG"
                >
                  💮 Stempel
                </button>
                <button
          ...
          <div ref={printAreaRef} id="printable-kk-content" className="space-y-6 bg-white p-2">
                ...
                <table className="w-full text-[11px] border-collapse border border-slate-300 print:border-black text-left">
                  <thead>
                    <tr className="bg-slate-100 print:bg-slate-200 text-slate-900 print:text-black font-extrabold">
                      ...
                      <th className="border border-slate-300 print:border-black p-2">SAKRAMEN PERKAWINAN</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 print:divide-black">
                    {members.map((ak, idx) => (
                      <tr key={`sak-${ak.id || idx}`} className="hover:bg-slate-50">
                        ...
                        <td className="border border-slate-300 print:border-black p-2">
                          {ak.baptis?.nama_baptis ? (
                            <div>
                              <div className="font-bold">{ak.baptis.nama_baptis}</div>
                              <div className="text-[10px] text-slate-600 print:text-black">Tgl: {ak.baptis.tgl_baptis || '-'} • Surat: {ak.baptis.no_surat_baptis || '-'}</div>
                            </div>
                          ) : (
                        ...
                        <td className="border border-slate-300 print:border-black p-2">
                          {ak.komuni_pertama?.tgl_komuni_pertama ? (
                            <div>
                              <div className="font-bold">{ak.komuni_pertama.tgl_komuni_pertama}</div>
                              <div className="text-[10px] text-slate-600 print:text-black">{ak.komuni_pertama.tempat_komuni_pertama || '-'}</div>
                            </div>
                          ) : (
                            <span className="text-slate-400 print:text-black italic">Belum Ada</span>
                          )}
                        ...
                        <td className="border border-slate-300 print:border-black p-2">
                          {ak.krisma?.nama_krisma ? (
                            <div>
                              <div className="font-bold">{ak.krisma.nama_krisma}</div>
                              <div className="text-[10px] text-slate-600 print:text-black">Tgl: {ak.krisma.tgl_krisma || '-'}</div>
                            </div>
                          ) : (
                            <span className="text-slate-400 print:text-black italic">Belum Ada</span>
                          )}
                        ...
                        <td className="border border-slate-300 print:border-black p-2">
                          {ak.perkawinan?.tgl_perkawinan ? (
                            <div>
                              <div className="font-bold">{ak.perkawinan.tgl_perkawinan}</div>
                              <div className="text-[10px] text-slate-600 print:text-black">{ak.perkawinan.tempat_perkawinan || '-'}</div>
                            </div>
                          ) : (
            ...
            <div className="pt-6 grid grid-cols-2 gap-8 text-xs font-semibold text-center print:pt-10">
              <div className="space-y-12">
                ...
                <div className="pt-8">
                  <p className="font-black underline uppercase">Antonius Bambang Setiawan</p>
                  <p className="text-[10px] text-slate-500 print:text-black">Ketua Lingkungan</p>
                </div>
              </div>
