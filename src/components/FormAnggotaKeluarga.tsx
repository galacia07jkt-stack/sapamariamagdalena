...
import { calculateAge, calculateAgeDetail, formatDateToYYYYMMDD, formatDateToDDMMYYYY, parseBirthDate, generateNIK } from '../lib/helpers';
import { UserPlus, Check, X, Lock, Heart, GraduationCap, Briefcase, Church, Calendar, Sparkles } from 'lucide-react';
...
export const FormAnggotaKeluarga: React.FC<FormAnggotaProps> = ({
  ...
  const [tanggalLahir, setTanggalLahir] = useState(
    initialData?.tanggal_lahir ? formatDateToYYYYMMDD(initialData.tanggal_lahir) : ''
  );
  const [tglDay, setTglDay] = useState('');
  const [tglMonth, setTglMonth] = useState('');
  const [tglYear, setTglYear] = useState('');
  ...
  const syncDateFromYMD = (ymd: string) => {
    setTanggalLahir(ymd);
    if (ymd && ymd.includes('-')) {
    ...
    if (ymd) {
      setCalculatedAge(calculateAge(ymd));
    } else {
      setCalculatedAge(0);
    }
  };

  // Auto NIK generation on open if new & Sync Birth Date
  useEffect(() => {
    if (isOpen) {
      const ymd = initialData?.tanggal_lahir ? formatDateToYYYYMMDD(initialData.tanggal_lahir) : '';
      syncDateFromYMD(ymd);
  ...
  }, [isOpen, idKK, noKK, initialData?.id, initialData?.tanggal_lahir, nik]);

  if (!isOpen) return null;

  const updateFullDate = (d: string, m: string, y: string) => {
    setTglDay(d);
    setTglMonth(m);
    setTglYear(y);
    ...
    if (d.length === 2 && m.length === 2 && y.length === 4) {
      ...
      const parsed = parseBirthDate(formattedDDMMYYYY);
      if (parsed) {
        const ymd = formatDateToYYYYMMDD(formattedDDMMYYYY);
        setTanggalLahir(ymd);
        setCalculatedAge(calculateAge(ymd));
      } else {
        setTanggalLahir('');
  ...
  const handleDayChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    updateFullDate(val, tglMonth, tglYear);
    if (val.length === 2) {
  ...
  const handleMonthChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    let val = e.target.value.replace(/\D/g, '');
    if (val.length > 2) val = val.slice(0, 2);
    updateFullDate(tglDay, val, tglYear);
    if (val.length === 2) {
  ...
  const handleMonthKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === '/' || e.key === 'Enter') {
      e.preventDefault();
      yearRef.current?.focus();
    } else if (e.key === 'Backspace' && !tglMonth) {
  ...
  const handleYearChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    updateFullDate(tglDay, tglMonth, val);
  };

  const handleYearKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Backspace' && !tglYear) {
      monthRef.current?.focus();
    }
  };

  const handlePasteDate = (e: React.ClipboardEvent<HTMLInputElement>) => {
    e.preventDefault();
    ...
    if (pasted.length >= 8) {
      ...
      updateFullDate(d, m, y);
      yearRef.current?.focus();
    } else if (pasted.length >= 2) {
      const d = pasted.slice(0, 2);
      updateFullDate(d, tglMonth, tglYear);
      monthRef.current?.focus();
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!tanggalLahir) {
      setError('Tanggal Lahir Wajib diisi dengan lengkap (DD/MM/YYYY)!');
      return;
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">
                  ...
                  <span className="text-[10px] font-medium text-slate-500 ml-1.5">(Tanda / permanen. Otomatis pindah saat terisi)</span>
                </label>
                
                <div className="relative flex items-center border border-slate-300 rounded-xl px-3 py-2 bg-white focus-within:ring-2 focus-within:ring-orange-500 focus-within:border-orange-500 transition-all shadow-xs">
                  {/* Day (TGL) Input */}
                  <input
                    ref={dayRef}
                    type="text"
                  ...
                  {/* Permanent Slash 1 */}
                  <span className="text-slate-400 font-black select-none mx-1 text-xs">/</span>
                  ...
                  {/* Calendar Icon Button Helper & Overlay Date Picker */}
                  <div className="ml-auto flex items-center pl-2 border-l border-slate-200 relative">
                    <button
                      type="button"
                      onClick={openDatePicker}
                      title="Klik untuk memilih dari Kalender"
                      className="p-1 text-orange-600 hover:bg-orange-50 rounded-lg transition-colors cursor-pointer flex items-center gap-1 text-[11px] font-bold"
                    >
